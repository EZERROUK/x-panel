...
import { UploadCloud, X, ArrowLeft, Save, AlertCircle, Edit3, Plus, Calendar } from 'lucide-react'
...
export default function EditProduct({
  ...
  const form = useForm<ProductFormData>({
    ...
    min_tolerance_type: product.min_tolerance_type || '',
  ...
  const restoreImage = (imgId: number) => {
    setData('deleted_image_ids', data.deleted_image_ids.filter(id => id !== imgId))
                      ...
                      {allImages.length > 0 && (
                        <motion.div layout className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            ...
                            return (
                              ...
                              >
                                <img
                                  ...
                                  className="h-32 w-full object-cover rounded-lg border border-slate-200 dark:border-slate-700"
                                  alt=""
                                />
                                <Button
                                  variant="ghost"
                                  ...
                                  type="button"
                                >
                                  {isDeleted ? <Plus className="w-4 h-4" /> : <X className="w-4 h-4" />}
                                </Button>
                                {!isDeleted && (
                                  <Button
                                    type="button"
                          ...
                          {newPreviews.map((src, i) => {
                            const visibleExistingCount = existingImages.filter(img => !data.deleted_image_ids.includes(img.id)).length
                                ...
                                <strong>Logique de compatibilité :</strong><br />
                                 {currentCategory && (
                                  <span>
                                    ...
                                    {parentCategory ? ` rattachée directement à la catégorie parente "${parentCategory.name}".` : ' (catégorie racine).'}
                                  </span>
                                )}
                              </p>
                            </div>
