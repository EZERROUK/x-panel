...
export default function ShowCurrency({ currency }: Props) {
  ...
  const { can } = useCan()
  const permsKey: string =
    (usePage() as any).props?.auth?.permissions?.join(',') ?? ''

  const canEdit    = can('currency_edit')    && !isDeleted
  const canRestore = can('currency_restore') &&  isDeleted

  const handleRestore = () => {
    if (!canRestore) { alert('Permission manquante: currency_restore'); return }
  ...
  return (
    <>
      ...
      >
        <div className="p-6">
          {/* Header */}
          <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6" key={permsKey}>
