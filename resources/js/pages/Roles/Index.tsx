...
const useCan = (isSuperAdminProp: boolean) => {
  ...
  const permsSet = useMemo(() => new Set(userPerms), [userPerms.join(',')]);
...
export default function RolesIndex({ roles, permissions, isSuperAdmin, flash }: Props) {
  const { can, isSuperAdmin: isSA } = useCan(isSuperAdmin);
  ...
  const showCheckboxHeader =
    showBulkBar &&
    paginatedRoles.some((r) => (!r.deleted_at && canBulkDelete) || (r.deleted_at && canBulkRestore));
  ...
  const deleteSelectedRoles = () => {
    ...
    Promise.all(
      selectedRoleIds.map((id) => router.delete(route('roles.destroy', { id }), { preserveScroll: true })),
    )
  ...
  const allSelected =
    paginatedRoles.length > 0 && selectedRoleIds.length === paginatedRoles.length;
                      ...
                      <>
                        {anySelectedInactive && canBulkRestore && (
                          <Button variant="secondary" onClick={restoreSelectedRoles}>
                            <RotateCcw className="w-4 h-4 mr-1" /> Restaurer ({selectedRoleIds.length})
                          </Button>
                        )}
                        {anySelectedActive && canBulkDelete && (
                          <Button variant="destructive" onClick={deleteSelectedRoles}>
                            <Trash2 className="w-4 h-4 mr-1" /> Supprimer ({selectedRoleIds.length})
                          </Button>
                        )}
                      </>
                    )}
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                  {paginatedRoles.length ? (
                    paginatedRoles.map((role) => {
                      const isActive = !role.deleted_at;
                      const canSelect =
                        (isActive && canBulkDelete) || (!isActive && canBulkRestore);
                      const rowHasAction = role.deleted_at
                        ? canRestore
                      ...
                      return (
                        <tr
                          key={role.id}
                          className={`${role.deleted_at ? 'bg-red-50 dark:bg-red-900/10' : ''} hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors`}
                        >
                          {showCheckboxHeader && (
