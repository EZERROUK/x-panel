...
export default function TaxRatesIndex({ taxRates, flash }: Props) {
  ...
  const allSelected = paginatedTaxRates.length > 0 && selectedTaxRateIds.length === paginatedTaxRates.filter((r)=>{
    const isActive = !r.deleted_at;
    if (isActive && !canBulkDelete) return false;
    if (!isActive && !canBulkRestore) return false;
    return true;
  }).length && paginatedTaxRates.every((r)=> !r.deleted_at ? selectedTaxRateIds.includes(r.id) || !canBulkDelete : selectedTaxRateIds.includes(r.id) || !canBulkRestore);
                      ...
                      <>
                        {anySelectedInactive && canBulkRestore && (
                          <Button variant="secondary" onClick={restoreSelectedTaxRates}>
                            <RotateCcw className="w-4 h-4 mr-1" /> Restaurer ({selectedTaxRateIds.length})
                          </Button>
                        )}
                        {anySelectedActive && canBulkDelete && (
                          <Button variant="destructive" onClick={deleteSelectedTaxRates}>
                            <Trash2 className="w-4 h-4 mr-1" /> Supprimer ({selectedTaxRateIds.length})
                          </Button>
                        )}
                      </>
                    )}
                  ...
                  {paginatedTaxRates.length ? (
                    paginatedTaxRates.map((taxRate) => {
                      ...
                      return (
                        <tr
                          key={taxRate.id}
                          className={`${taxRate.deleted_at ? 'bg-red-50 dark:bg-red-900/10' : ''} hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors`}
                        >
                          {/* checkbox */}
                          {showCheckboxHeader && (
                          ...
                          {showActionsColumn && (
                            <td className="px-6 py-4 text-center">
                              {showRowActions ? (
                                ...
                                <span className="text-slate-400 dark:text-slate-600 text-xs italic">Aucune action</span>
                              )}
                            </td>
                          )}
                        </tr>
